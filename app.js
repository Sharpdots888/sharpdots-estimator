...
els.reportProjectTitle.textContent = els.projectName.value;
...
function readSavedEstimates() {
  try {
    const stored = localStorage.getItem(estimateStoreKey);
    if (!stored) return savedEstimateCache;
    const saved = JSON.parse(stored);
    if (Array.isArray(saved)) savedEstimateCache = saved;
    return savedEstimateCache;
  } catch {
    return savedEstimateCache;
  }
}
...
function writeSavedEstimates(estimates) {
  savedEstimateCache = structuredClone(estimates);
  try {
...
function sortSavedEstimates(estimates) {
...
function nextProjectNumber() {
  const highest = readSavedEstimates().reduce((max, estimate) => Math.max(max, projectNumberSortValue(estimate.projectNumber)), 0);
...
function findEstimateByProjectNumber(projectNumber) {
  return readSavedEstimates().find((estimate) => String(estimate.projectNumber || "").trim() === String(projectNumber || "").trim());
}

function findEstimateByProjectName(projectName) {
  const matches = sortSavedEstimates(readSavedEstimates()).filter((estimate) => estimateMatchesName(estimate, projectName));
  return matches.at(-1);
}
...
function renderDatalists() {
  const savedEstimates = sortSavedEstimates(readSavedEstimates());
  [
...
function renderLoadEstimateList() {
  ...
  const estimates = sortSavedEstimates(readSavedEstimates()).filter((estimate) => !query || estimateSearchText(estimate).includes(query));
...
function saveCurrentEstimate() {
  ...
  const estimates = readSavedEstimates().filter((saved) => saved.projectNumber !== projectNumber);
  estimates.push(estimate);
  const persisted = writeSavedEstimates(sortSavedEstimates(estimates));
  ...
  render();
  setSaveStatus(persisted ? `Saved ${projectNumber}` : `Saved for this session ${projectNumber}`);
}
