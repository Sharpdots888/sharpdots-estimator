...
async function getCatalog({ includeDraft = false } = {}) {
  ...
  const [versionsResult, categoriesResult, productsResult, groupsResult, valuesResult, configurationsResult, tiersResult, configurationValuesResult, optionalsResult, optionalTiersResult, applicabilityResult] = await Promise.all([
    ...
    db.query(`SELECT id, category_id, product_code, name, description, default_markup_percent, source_external_id, status FROM sdsp_products WHERE status ${statusFilter} ORDER BY name`),
...
async function calculatePrice(payload) {
  ...
  const quantity = Math.max(Math.round(toNumber(payload.quantity)), 1);
  ...
  const markupPercent = Math.min(Math.max(toNumber(payload.markupPercent), 0), 200);
